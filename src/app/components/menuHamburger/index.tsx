...
export default function Hamburger({ open, setOpen }: { open: boolean; setOpen: (value: boolean) => void }) {
  ...
  return (
    <div className={`fixed right-[40px] w-[50px] h-[50px] justify-center items-center rounded-full cursor-pointer flex transition-all duration-300 ease-in-out z-30 hoverSeta ${scrolled ? "bg-[#00BFFF]" : "bg-transparent"} div-menu`}>
      <div className={`absolute flex justify-center items-center w-0 h-0 bg-[#00BFFF] transition-all duration-600 ease-in-out rounded-[100%] ${open ? "right-[-40px] top-[-45px] w-[100vw] h-[100vh] rounded-none" : ""} menu-w`}>
        <div className={`${open ? "flex flex-col justify-start w-[1000px] z-30 gap-[40px]" : "hidden"}`}>
      ...
      <button
        ...
        className="relative w-[30px] h-[30px] cursor-pointer transition-all duration-500 ease-in-out hoverSeta z-40"
      >
