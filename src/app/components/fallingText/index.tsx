...
export default function FallingText({ text }: { text: string }) {
  ...
  <div ref={containerRef} className="flex z-999">
        {text.split("").map((letter, i, arr) => (
          <h3 key={i}
            className={`text-[38px] text-[#F5F5F5] font-semibold transition-all duration-300 ease-in-out hoverSeta ${visible ? "translate-y-0 opacity-100" : "-translate-y-[100px] opacity-0"}`}
            style={{
